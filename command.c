...
#ifdef STUDENT
    size_t next_len; // length till next ':' character
    while ((next_len = strcspn(path, ":")) > 0) {
      char *full_path = strndup(path, next_len);

      strapp(&full_path, "/");
      strapp(&full_path, argv[0]); // append current program's name

      (void)execve(full_path, argv, environ);
      free(full_path); // strapp uses realloc so we free here

      path += (next_len + 1); // one for ':' char
    }
#endif /* !STUDENT */
  } else {
